...
class EmbeddingCache:
    ...
    def _initialize(cls):
        """
        Phương thức này sẽ khởi tạo manager và các tài nguyên dùng chung
        chỉ khi chúng được gọi lần đầu tiên.
        """
        if cls._manager is None:
            print("Initializing multiprocessing.Manager for the first time...")
            cls._manager = multiprocessing.Manager()
            cls._cache = cls._manager.dict()
            cls._lock = cls._manager.Lock()

    def get(self, key):
        self._initialize()  # Đảm bảo manager đã được khởi tạo
        with self._lock:
