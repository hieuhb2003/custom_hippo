...
class BaseConfig:
    ...
    # Chunking (runtime indexing) attributes
    enable_chunking: bool = field(
        default=True,
        metadata={"help": "Enable token-based chunking at indexing time."},
    )
    chunk_tokens: int = field(
        default=800, metadata={"help": "Max tokens per chunk (tiktoken)."}
    )
    chunk_overlap_tokens: int = field(
        default=200, metadata={"help": "Overlap tokens between chunks (tiktoken)."}
    )
    chunk_encoding: str = field(
        default="o200k_base",
        metadata={
            "help": "tiktoken encoding name (e.g., o200k_base for gpt-4o/4o-mini)."
        },
    )
